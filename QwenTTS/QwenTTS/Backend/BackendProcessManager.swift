...
class BackendProcessManager {
    // M7:依赖协议缝而非具体类(生产 adapter=BackendLauncher 包 Process;
    // 测试可注入 fake 驱动崩溃路径不 spawn 真进程)
    private let launcher: BackendLaunching
    private(set) var apiClient: BackendAPIClient?

    init(launcher: BackendLaunching = BackendLauncher()) {
    ...
    func seedDevEnvironmentIfNeeded() -> Bool {
        ...
        let pythonCandidates = [
            ...
            workspaceRoot.appendingPathComponent(".venv/bin/python").path,
            workspaceRoot.appendingPathComponent(".venv/bin/python3").path,
    ...
    private func updateState(_ newState: BackendState) {
        ...
        if newState == .ready {
            ...
            // #10 C7:列表取数/轮询收口到 store(与快照轮询同一个 client)
            stateStore?.listClient = apiClient
            stateStore?.startListPolling()
        } else {
            snapshotTask?.cancel()
            snapshotTask = nil
            stateStore?.stopListPolling()
            // 复核 P2-6:后端非 ready 时清掉列表 client,避免重启窗口期
            // 手动切 tab 对死后端发请求(5s 超时白等);ready 后重注入。
            stateStore?.listClient = nil
        }
    }
    ...
    private func handleProcessExit() {
        ...
        if state == .stopping {
            ...
            // M7:异常退出的重启决策(退避/上限/稳定清零)归 RestartPolicy 纯算术,
            // 这里只执行副作用(置状态、定时重启)。
            let decision = RestartPolicy.onAbnormalExit(
                lastReadyAt: lastReadyTime, restartCount: restartCount, now: Date()
            )
            restartCount = decision.newRestartCount
            updateState(.failed)
            switch decision.action {
            case .restart(let backoffDelay):
                print("[ProcessManager] Backend crashed. Restarting in \(backoffDelay)s (Attempt \(restartCount)/\(RestartPolicy.maxAttempts)).")

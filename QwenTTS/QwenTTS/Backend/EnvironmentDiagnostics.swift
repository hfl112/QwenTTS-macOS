...
enum EnvironmentDiagnostics {
    ...
    static func run() -> [Item] {
        ...
        if modelInstalled(ModelManager.defaultModelName) {
            items.append(Item(name: "模型", status: .ok, detail: "\(ModelManager.defaultModelName) 已安装", fixHint: nil))
            ...
            items.append(Item(name: "模型", status: .fail,
                              detail: "未安装任何 TTS 模型",
                              fixHint: "下载 \(ModelManager.defaultModelName)（推荐），或选择本机已有的模型目录。"))
        }

        // 6. 参考音频（ICL 锁音所需）
        let refDir = resolved(custom: cfg.referenceAudioPath, bundleSubpath: "/Backend/reference")
        let required = ["ref_serena_en.wav", "ref_ryan.wav", "ref_serena_zh.wav"]

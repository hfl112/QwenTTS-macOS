...
final class ModelManager: NSObject {
    ...
    // 目录名必须与后端 constants.py 的 DEFAULT_TTS_MODEL 完全一致,否则向导下载
    // 完成后后端仍找不到模型(§4l:必须用 4bit 量化版,bf16 无法实时)。
    static let defaultModelName = "Qwen3-TTS-0.6B-4bit"
    static let defaultModelRepoID = "mlx-community/Qwen3-TTS-12Hz-0.6B-Base-4bit"
    ...
    var modelsDirectory: URL {
        // #12+:自定义环境(dev)下模型在 TTS_MODELS_PATH 指向的目录——安装状态
        // 检查/下载落盘必须与后端实际加载目录一致,否则设置页显示与事实脱节。
        if EnvironmentConfigManager.shared.mode == .custom {
            let custom = EnvironmentConfigManager.shared.customConfig.modelsPath
            if !custom.isEmpty {
    ...
    /// 目的目录当前已落盘的字节数(含 huggingface_hub 在 local_dir/.cache 下的
    /// 半成品分片)。app 侧拿不到 hub 的真实传输进度,以体积近似供 UI 展示。
    func downloadedBytes(name: String) -> Int64 {

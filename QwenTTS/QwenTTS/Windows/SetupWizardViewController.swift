...
class SetupWizardViewController: NSViewController {
    ...
    private func setupUI() {
        ...
        configureButton(downloadButton, "下载 \(ModelManager.defaultModelName)（推荐）", #selector(downloadClicked))
    ...
    @objc private func downloadClicked() {
        ...
        // huggingface_hub 掌握真实传输进度,app 侧拿不到百分比——此前把进度条
        // 停在 1% 的假进度让用户误以为卡死。改为不定长动画 + 每秒轮询落盘体积。
        progressIndicator.isIndeterminate = true
        progressIndicator.isHidden = false
        progressIndicator.startAnimation(nil)
        infoLabel.stringValue = "下载中…正在连接"
        downloadPollTimer?.invalidate()
        downloadPollTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
            let bytes = ModelManager.shared.downloadedBytes(name: ModelManager.defaultModelName)
            let mb = Double(bytes) / 1_048_576
            self?.infoLabel.stringValue = mb < 1
                ? "下载中…正在连接"
                : String(format: "下载中…已接收 %.0f MB(约 1.7 GB)", mb)
        }
        let repoID = ModelManager.defaultModelRepoID
        ModelManager.shared.startDownload(name: ModelManager.defaultModelName, repoID: repoID, progress: { _ in
            // 真实进度由上面的轮询计时器展示
    ...
    @objc private func selectModelClicked() {
        ...
        panel.prompt = "选择模型目录"
        panel.message = "请选择包含 model.safetensors 与 config.json 的模型目录（如 \(ModelManager.defaultModelName)）"
    ...
    @objc private func downloadHelpClicked() {
        ...
        1. 点「下载 \(ModelManager.defaultModelName)」由 App 自动下载（推荐，需联网）。

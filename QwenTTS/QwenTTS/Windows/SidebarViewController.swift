...
class SidebarViewController: NSViewController {
    ...
    // 全局「后台播客生成中」指示:直接改「内容中心」按钮标题(播客在该页),
    // 任何 tab 都可见。由 MainSplitVC 订阅 active_podcast_processes 驱动。
    private static let libraryTabIndex = 1
    private static let libraryBaseTitle = "内容中心"
    ...
    /// 由 MainSplitViewController 在每次快照后调用:count>0 时「内容中心」标题带 🎙️N。
    func updatePodcastGenerating(count: Int) {
        guard Self.libraryTabIndex < buttons.count else { return }
        let btn = buttons[Self.libraryTabIndex]
        let wanted = count > 0 ? "\(Self.libraryBaseTitle)  🎙️\(count)" : Self.libraryBaseTitle
        if btn.title != wanted { btn.title = wanted }
    }

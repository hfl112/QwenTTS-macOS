...
struct UITester {
    static func dumpViewHierarchy() {
        ...
        let path: String
        if let envPath = ProcessInfo.processInfo.environment["QWENTTS_UI_DUMP_PATH"] {
            path = envPath
        } else {
            path = (FileManager.default.currentDirectoryPath as NSString).appendingPathComponent("qwentts_ui_dump.txt")
        }
        do {

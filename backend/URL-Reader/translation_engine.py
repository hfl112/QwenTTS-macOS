...
def translate_text(text: str, target: Optional[str] = None) -> str:
    ...
    # LLM-first：配置了可用 LLM 时，普通翻译也优先走 LLM（质量更高，且避开免费
    # Google 接口在限流时"原样回吐英文"导致整篇不翻译的问题）。LLM 失败再退回
    # 下面的机器翻译供应商（google/microsoft/deepl）。
    try:
        from llm_engine import llm_selected_available
        if llm_selected_available():
            order = ["llm"] + [o for o in order if o != "llm"]
    except Exception:
        pass
    tried: List[str] = []
    ...
    for name in order:
        if name == "llm":
            tried.append("llm")
            try:
                print("[Translate] using LLM engine for translation")

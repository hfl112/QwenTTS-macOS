...
* **控制说明文档**：`README.md`（本文件）
* **可复用输入管线**：`reader_service.py`
* **独立命令行测试工具**：`read_url_cli.py`（薄 CLI 壳，复用 `reader_service.py`）
...
### B. 执行网页朗读 / 生成
在本目录下，直接在终端中运行 `read_url_cli.py` 并传入网页 URL。默认模式是 `--podcast-discuss`，会先通过 Gemini 生成双人总结稿，再投喂给 QwenTTS 后端。

```bash
cd <repo>/localTTS_macOS/backend/URL-Reader

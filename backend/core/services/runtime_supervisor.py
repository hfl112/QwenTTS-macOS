...
class RuntimeSupervisor:
    ...
    def _close_queues(self) -> None:
        for queue_name in ("text_q", "audio_q", "podcast_q"):
    ...
    def start_watchdog(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        def arm_hard_exit() -> None:
            """E7 F-b:EOF 后的最后防线——宽限期内进程必须退出,否则 os._exit 硬退。

            仅真实部署(TTS_WATCHDOG_EXIT_PROCESS=1)生效。优雅路径先退出时,
            守护线程随进程消亡,不影响正常关闭;uvicorn 被在途请求挂住时,
            这里保证不留孤儿。"""
            if os.environ.get("TTS_WATCHDOG_EXIT_PROCESS") != "1":
                return
            try:
                grace = float(os.environ.get("TTS_WATCHDOG_HARD_EXIT_SEC", "10"))
            except ValueError:
                grace = 10.0

            def hard_exit() -> None:
                time.sleep(grace)
                self._record("watchdog_hard_exit", grace_sec=grace)
                os._exit(1)

            threading.Thread(
                target=hard_exit, name="watchdog-hard-exit", daemon=True
            ).start()
        ...
        async def shutdown_after_watchdog() -> None:
            try:
                ...
                # E7 F-a:无论 shutdown 是否抛异常,自杀必发——否则主进程成孤儿
                if os.environ.get("TTS_WATCHDOG_EXIT_PROCESS") == "1":
        ...
        def watchdog_loop() -> None:
            ...
            try:
                # E7 F-c:循环读到 EOF 为止。读到数据字节(当前无人写,防未来
                # 心跳)只忽略,绝不静默罢工。
                while True:
                    data = os.read(fd, 1)
                    if not data:

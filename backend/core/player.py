...
class PCMPlayer:
    ...
    def __init__(self, sample_rate: int = 24000) -> None:
        ...
        # Audio callback blocksize. Larger = coarser underrun gaps (8192 @ 24kHz ≈ 341ms
        # of silence per starve); smaller = shorter, less audible gaps at the cost of more
        # callbacks. TTS_PLAYER_BLOCKSIZE overrides for the underrun A/B; default 8192 =
        # current behavior.
        self.blocksize: int = 8192
        _env_block = os.environ.get("TTS_PLAYER_BLOCKSIZE")
        if _env_block is not None:
            try:
                self.blocksize = max(64, int(_env_block))
            except ValueError:
                pass
        ...
        self._queue_duration_sec: float = 0.0
    ...
    def _callback(self, outdata: np.ndarray, frames: int, time_info: Any, status: Any) -> None:
        ...
        with self._lock:
            ...
            # Subtract the duration of samples currently sent to sound device
            self._queue_duration_sec -= filled / self.sample_rate
            if self._queue_duration_sec < 0.0:
                self._queue_duration_sec = 0.0
                
            outdata[:] = np.clip(data_to_fill * self.volume_scale, -0.98, 0.98)

    def get_queue_duration(self) -> float:
        with self._lock:
            return self._queue_duration_sec

    def start(self, speed: float = 1.0, prebuffer_frames: int | None = None) -> None:
        """开始一轮播放。prebuffer_frames:本轮起播水位(帧数);此前调用方靠
        「先戳 min_chunks_to_start 字段再 start()」的两步默契(#10 C4 收成显式参数)。"""
    ...
    def play_chunk(self, chunk: np.ndarray | tuple[np.ndarray, int]) -> None:
        ...
        with self._lock:
            self._queue_duration_sec += duration
    ...
    # ---- 方法契约(#10 C4):外部读播放器状态一律走这些方法,不摸字段 ----

    def queue_depth(self) -> int:
        """待播 chunk 数(audio_queue 深度);异常时 0。"""
        try:
            return self.audio_queue.qsize()
        except Exception:
            return 0

    def playing_index(self) -> Optional[int]:
        """当前正在出声的句索引;未在播/未知(<0 或 None)→ None。"""
        idx = self.currently_playing_index
        if idx is None or idx < 0:
            return None
        return idx

    def finish(self) -> None:
        """标记播放结束(等价旧的直接 set playback_finished_event)。"""
        self.playback_finished_event.set()

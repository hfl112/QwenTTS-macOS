...
def test_sentinel_passthrough(tmp_path):
    """End-of-article markers carry the originating task_id. A tagged marker for
    the current session — and the legacy bare sentinel — are forwarded tagged."""
    eng = _engine(tmp_path)
    sh = FakeShared(current_task_id=1)
    sh.text_q.put(("SENT", 1))     # tagged marker for the current session
    sh.text_q.put("SENT")          # legacy bare sentinel (defensive fallback)
    sh.text_q.put(None)

    eng.run_loop(sh, sentinel="SENT", profile_fn=_PROFILE)
    assert _drain(sh.audio_q) == [("SENT", 1), ("SENT", 1)]


def test_stale_sentinel_is_dropped(tmp_path):
    """Regression: a superseded session's end-of-article marker must NOT reach
    the audio queue. Previously the bare sentinel carried no identity, so a stale
    sentinel finished the NEW read prematurely (UI idle) while its chunks kept
    generating headless (GPU burn)."""
    eng = _engine(tmp_path)
    sh = FakeShared(current_task_id=5)
    sh.text_q.put(("SENT", 3))     # marker from old session 3 — must be dropped
    sh.text_q.put(None)
...
def test_read_lane_trims_chunk_silence(tmp_path):
    ...
    assert 0 < total < raw * 0.8, f"read lane did not trim silence (got {total}, raw {raw})"
...
def test_podcast_chunk_aborts_mid_synthesis_on_cancel(tmp_path):
    """P0-1(#10a):cancel_all bump 代际后,引擎中途掐断正在合成的播客段——
    不写 chunk 文件、不写 .err、不再烧后续帧(否则 quiet 档一段能白烧数分钟)。"""
    eng = _engine(tmp_path)
    sh = FakeShared()
    sh.podcast_cancel_epoch = _Val(0)

    # 包一层 backend:产出第 1 帧后"发生取消"(bump 代际),后续帧不该再被消费
    real = eng.backend
    consumed = {"n": 0}

    class CancelMidwayBackend:
        def __getattr__(self, name):
            return getattr(real, name)

        def generate(self, *a, **k):
            for i, frame in enumerate(real.generate(*a, **k)):
                consumed["n"] += 1
                if i == 0:
                    sh.podcast_cancel_epoch.value += 1  # 第 1 帧后取消
                yield frame

    eng.backend = CancelMidwayBackend()
    chunk_file = os.path.join(str(tmp_path), "chunk_cancel.npy")
    ...
    t.start()
    sh.podcast_q.put(
        {"job_id": "jc", "chunk_index": 0, "chunk_file": chunk_file,
         "text": "会被取消的一句", "config": {"voice": "Ryan"}}
    )
    time.sleep(0.5)
    sh.text_q.put(None)
    t.join(timeout=5)

    assert not os.path.exists(chunk_file), "取消后不得写出 chunk 文件"
    assert not os.path.exists(chunk_file + ".err"), "掐断不是错误,不写 .err"
    assert consumed["n"] <= 2, f"取消后不应继续消费帧(实际消费 {consumed['n']})"

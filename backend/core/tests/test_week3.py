...
def test_snapshot_article_view_byte_compat():
    """C3.2(#10):钉死 /snapshot 的文章视图三字段在「持久化索引 vs 实时索引
    覆盖」下的取值——C3.4 把逐请求对账换成单一拥有者合成时,本测试必须原样绿。"""
    import core.backend as backend_mod
    ...
    old_article = state.get("current_article")
    state["current_article"] = {
        "title": "对账测试",
        # dict-chunk 与 str-chunk 混合:wire 必须清洗成纯文本列表
        "chunks": [{"text": "第一句", "config": {}}, "第二句", "第三句"],
        "current_index": 1,
    }
    backend_mod.storage.save_state(state)
    try:
        # 场景 1:player 无实时索引(<0)→ 用持久化 index,main_progress 不被覆盖
        p.currently_playing_index = -1
        snap = client.get("/snapshot").json()
        assert snap["current_article_chunks"] == ["第一句", "第二句", "第三句"]
        assert snap["current_article_index"] == 1

        # 场景 2:实时索引在范围内 → 覆盖 index,并实时覆盖 main_progress
        p.currently_playing_index = 2
        snap = client.get("/snapshot").json()
        assert snap["current_article_index"] == 2
        assert snap["main_progress"] == "3/3"

        # 场景 3:实时索引越界 → 保持持久化 index(历史行为:只在 <len 时覆盖)
        p.currently_playing_index = 99
        snap = client.get("/snapshot").json()
        assert snap["current_article_index"] == 1
    finally:
        p.currently_playing_index = -1
        state["current_article"] = old_article or {"title": "", "chunks": [], "current_index": 0}
        backend_mod.storage.save_state(state)

...
def main() -> None:
    ...
    # 目录存在 ≠ 内容完整:mlx_audio 的架构源码在 .../models/ 下,曾整体被
    # gitignore 吞掉——干净 clone 打出的包目录仍在、但加载模型即崩。抽查现役
    # qwen3_tts 架构与 ICL 参考音频,防复发。
    # 注意:copy_python_sources 拷的是内层包(backend/mlx_audio/mlx_audio →
    # Backend/mlx_audio),bundle 里没有双层嵌套。
    qwen3_arch = (APP / "Contents/Resources/Backend/mlx_audio"
                  / "tts/models/qwen3_tts/qwen3_tts.py")
    check(qwen3_arch.is_file(), "qwen3_tts architecture source is present")
    for ref in ("ref_serena_en.wav", "ref_serena_zh.wav", "ref_ryan.wav"):
        check((APP / "Contents/Resources/Backend/reference" / ref).is_file(),
              f"ICL reference audio is present: {ref}")

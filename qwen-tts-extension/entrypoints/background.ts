export default defineBackground(() => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          return {
            result: { error: "配对码未配置或不正确：请在 QwenTTS App 设置中生成配对码，并粘贴到扩展的 Pairing Token" },
            networkError: false,
            staleInstance: false
          };
        }
      ...
      browser.tabs.query({ active: true, currentWindow: true }).then(([tab]) => {
        if (tab?.id) {
          browser.scripting.executeScript({
            ...
            if (clipText) {
              callBackend("/read", "POST", { text: clipText, source: "clipboard" })
  ...
  browser.contextMenus.onClicked.addListener(async (info, tab) => {
    if (info.menuItemId === "qwen-read-selection" && info.selectionText) {
      callBackend("/read", "POST", { text: info.selectionText, source: "clipboard" });
    }
  });
  ...
  browser.commands.onCommand.addListener(async (command) => {
    if (command === "qwen-tts-read") {
      ...
      if (tab?.id) {
        ...
        if (selectionText) {
          callBackend("/read", "POST", { text: selectionText, source: "clipboard" });
        }
      }
    }
  });

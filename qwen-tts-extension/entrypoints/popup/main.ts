...
const fetchSavedItems = async () => {
  ...
  try {
    ...
    if (response && !response.error && Array.isArray(response)) {
      ...
      // {error} = 连接失败,别误显示成"暂无内容"(#12-⑥ 用户实测反馈)
      savedList.innerHTML = '<div class="empty-state">后端未连接(App 未运行?)</div>';
    }
  } catch (err) {
...
const renderSavedList = (items: any[]) => {
  ...
  items.forEach((item, index) => {
    ...
    // 根据来源 source 生成指示标签(#11 N3:后端 source_label 优先——词表
    // 单一真相在后端 core/labels.py,与 App 同源;旧后端无该字段时回退本地推断)
    let sourceLabel = "[剪贴]";
    let sourceClass = "source-clipboard";
    if (item.source_label) {
      sourceLabel = `[${item.source_label}]`;
      sourceClass = item.source_label === "网页" ? "source-web"
        : item.source_label === "视频" ? "source-video" : "source-clipboard";
    ...
    // Snip text to fit(#11 N3:后端干净标题 display_title 优先,与 App 同源)
    let displayTitle = (item.display_title || item.title || item.text.trim()).replace(/\s+/g, ' ').trim();
    ...
    // #11 N3:模式标签(原文不标,与 App 词表同源:译文/双人总结/双人翻译)
    if (item.mode_label && item.mode_label !== "原文") {
      const modeEl = document.createElement('span');
      modeEl.className = 'cache-source-tag source-web';
      modeEl.textContent = `[${item.mode_label}]`;
      textEl.append(sourceEl, modeEl, titleEl);
    } else {
          ...
          try {
            if (currentPlayingMd5 === item.md5) {
              if (localIsPlaying && !localIsPaused) {
                const res = await callBackend("/pause", "POST");
                if (res?.error) {
                  alert(`❌ 暂停失败: ${res.error}`);
                  return;
                }
                applyCommandPlaybackStatus(res, "paused");
              } else {
                const res = await callBackend("/resume", "POST");
                if (res?.error) {
                  alert(`❌ 恢复失败: ${res.error}`);
                  return;
                }
                applyCommandPlaybackStatus(res, "playing");
              }
              lastActionTime = Date.now();
              updatePlayToggleUI();
              updateSavedListUI();
...
const fetchPodcasts = async () => {
  ...
  try {
    ...
    if (response && !response.error && Array.isArray(response)) {
      ...
      podcastList.innerHTML = '<div class="empty-state">后端未连接(App 未运行?)</div>';
    }
  } catch (err) {
...
const renderPodcastList = (items: any[]) => {
  ...
  items.forEach((item) => {
    ...
    // 根据来源生成指示标签(#11 N3:后端 source_label 优先,与 App 词表同源)
    let sourceTag = "";
    const srcColors: Record<string, string> = { "视频": "#3b82f6", "网页": "#8b5cf6", "剪贴板": "#10b981", "剪贴": "#10b981", "播客": "#f59e0b" };
    const legacySrc: Record<string, string> = { video: "视频", web: "网页", clipboard: "剪贴", podcast: "播客" };
    const srcLabel = item.source_label || legacySrc[item.source] || "";
    if (srcLabel) {
      sourceTag = `<span style="color: ${srcColors[srcLabel] || "#8b5cf6"}; font-weight: bold; margin-right: 4px;">[${srcLabel}]</span>`;
    }
    // 模式标签(原文不标;成品 mode 由后端从 job 记录反查,老文件无 → 不显示)
    if (item.mode_label && item.mode_label !== "原文") {
      sourceTag += `<span style="color: #a855f7; font-weight: bold; margin-right: 4px;">[${item.mode_label}]</span>`;
    }

    // 处理显示标题(#11 N3:后端干净标题优先)
    let displayTitle = "";
    if (item.display_title) {
      displayTitle = item.display_title.trim();
      if (displayTitle.length > 28) displayTitle = displayTitle.slice(0, 26) + '...';
    } else if (item.title && item.title !== (item.filename || item.title)) {
...
const fetchCaches = async () => {
  ...
  try {
    ...
    if (response && !response.error && Array.isArray(response)) {
      ...
      cacheList.innerHTML = '<div class="empty-state">后端未连接(App 未运行?)</div>';
    }
  } catch (err) {
...
const renderCacheList = (items: any[]) => {
  ...
  items.forEach((item) => {
    ...
    // 来源分类标签(后端 cache 现带 source;旧缓存无 source 时留空)
    const srcLabel =
      item.source === "url" || item.source === "web" ? "网页" :
      item.source === "video" ? "视频" :
      item.source === "clipboard" ? "剪贴板" :
      item.source === "selection" ? "选中" :
      (item.source || "");
    ...
    itemEl.innerHTML = `
      <div class="cache-info">
        ...
        <div class="cache-meta">
          ${srcLabel ? `<span>${srcLabel}</span><span>·</span>` : ""}
...
const applyPlaybackSnapshot = (snapshot: any): boolean => {
  if (!snapshot || snapshot.error) return false;

  const previous = {
    playing: localIsPlaying,
    paused: localIsPaused,
    podcast: currentPlayingPodcastFile,
    md5: currentPlayingMd5,
  };

  const status = snapshot.playback_status;
  if (status === "playing" || status === "generating") {
    ...
    localIsPlaying = Boolean(snapshot.is_playing);
    localIsPaused = Boolean(snapshot.is_paused);
  }

  const active = localIsPlaying || localIsPaused;
  currentPlayingPodcastFile = active ? (snapshot.current_podcast_file || currentPlayingPodcastFile) : null;
  currentPlayingMd5 = active ? (snapshot.current_playing_md5 || currentPlayingMd5) : null;

  return previous.playing !== localIsPlaying
    || previous.paused !== localIsPaused
    || previous.podcast !== currentPlayingPodcastFile
    || previous.md5 !== currentPlayingMd5;
};

const applyCommandPlaybackStatus = (response: any, fallback: "playing" | "paused" | "idle") => {
  if (response?.playback_status) {
    applyPlaybackSnapshot(response);
    return;
  }
  if (fallback === "idle") {
    localIsPlaying = false;
    ...
    localIsPlaying = true;
    localIsPaused = fallback === "paused";
  }
};

const syncPlaybackStatusNow = async () => {
  const res = await callBackend("/status");
  if (res && !res.error) {
    applyPlaybackSnapshot(res);
    updatePlayToggleUI();
    updatePodcastListUI();
    updateSavedListUI();
  }
...
const updatePlayToggleUI = () => {
  ...
  btnPlayToggle.title = localIsPlaying && localIsPaused ? "继续播放" : "直接朗读当前 URL";
...
btnPlayToggle.onclick = async () => {
  ...
  try {
    await syncPlaybackStatusNow();

    if (localIsPlaying && !localIsPaused) {
      // 播放和暂停拆成独立键：正在播放时，播放键不再承担暂停语义。
      return;
...
btnPause.onclick = async () => {
  try {
    await syncPlaybackStatusNow();
    if (!localIsPlaying || localIsPaused) return;
...
btnStop.onclick = async () => {
  ...
  try {
    const res = await callBackend("/stop", "POST");
    if (res?.error) {
      alert(`❌ 停止失败: ${res.error}`);
      return;
    }
    applyCommandPlaybackStatus(res, "idle");
    lastActionTime = Date.now();
    updatePlayToggleUI();
    updateSavedListUI();
    updatePodcastListUI();
...
// 定时 1Hz 轮询更新状态
// #12-⑥ 陈旧"生成中"修复:此前后端失联时所有 catch 静默吞掉,面板永远冻结在
// 最后一帧(含"⏳ 生成中..."占位行)。现在 /status 连续 3 次失败 → 三个列表
// 切到"后端未连接"空态并清缓存指纹;恢复后自动整体重载。
...
const markBackendOffline = () => {
  ...
  lastPodcastsData = "";
  lastCachesData = "";
  const offlineHtml = '<div class="empty-state">后端未连接(App 未运行?)</div>';
  savedList.innerHTML = offlineHtml;
  podcastList.innerHTML = offlineHtml;
  cacheList.innerHTML = offlineHtml;
};
...
const startStatusPolling = () => {
  ...
  setInterval(async () => {
    pollTick++;
    if (pollTick % 2 === 0 && !backendOffline) {
    ...
    try {
      ...
      const res = await callBackend("/status");
      // 注意:callBackend 经 background 转发,连接失败**不抛异常**而是返回
      // {error: "Command failed"} —— 失败判定必须看 res.error,不能只靠 catch。
      if (res && !res.error) {
        statusFailStreak = 0;
        if (backendOffline) {
          // 失联恢复:整体重载三个列表(缓存指纹已清,必然重渲染)
          backendOffline = false;
          fetchSavedItems();
          fetchPodcasts();
          fetchCaches();
        }
        if (applyPlaybackSnapshot(res)) {
          updatePlayToggleUI();
          updatePodcastListUI();
          updateSavedListUI();
        }
        ...
        statusFailStreak++;
        if (statusFailStreak >= 3) markBackendOffline();
      }
    } catch {
      statusFailStreak++;
      if (statusFailStreak >= 3) markBackendOffline();
